...
async function main() {
  ...
  if (aliciaPartner) {
    ...
    if (!existingTripForm) {
      await prisma.tripForm.create({
        data: {
          partner: { connect: { id: aliciaPartner.id } },
          fullName: 'Alicia Lim',
  ...
  const healthProjectDate = addDays(anchorDate, 10);
  const healthProject = await getOrCreateVolunteerProject({
    managedBy: { connect: { id: generalManager.id } },
    approvedBy: { connect: { id: generalManager.id } },
  ...
  const mentorshipDate = addDays(anchorDate, 18);
  ...
  const warehouseDate = addDays(anchorDate, -14);
  ...
  const aliciaUser = requireUser('alicia.partner@siloam.org');
  const rahulUser = requireUser('rahul.partner@siloam.org');
  ...
  if (healthRegistration) {
    await prisma.volunteerProjectPosition.upsert({
      ...
      update: {},
      create: {
        volunteer: { connect: { id: aliciaUser.id } },
        position: { connect: { id: healthRegistration.id } },
        status: VolunteerProjectPositionStatus.approved,
        hasConsented: true,
        approvedAt: new Date(),
        approver: { connect: { id: generalManager.id } },
  ...
  if (mentorshipMentor) {
    await prisma.volunteerProjectPosition.upsert({
      ...
      update: {},
      create: {
        volunteer: { connect: { id: rahulUser.id } },
        position: { connect: { id: mentorshipMentor.id } },
  ...
  if (warehouseSorting) {
    await prisma.volunteerProjectPosition.upsert({
      ...
      update: {},
      create: {
        volunteer: { connect: { id: aishaUser.id } },
        position: { connect: { id: warehouseSorting.id } },
  ...
  if (healthSession) {
    await prisma.volunteerSession.upsert({
      ...
      create: {
        volunteer: { connect: { id: aliciaUser.id } },
        session: { connect: { id: healthSession.id } },
        approvedAt: new Date(),
        approver: { connect: { id: generalManager.id } },
        has_rsvp: true,
      },
  ...
  if (mentorshipSession) {
    await prisma.volunteerSession.upsert({
      ...
      create: {
        volunteer: { connect: { id: rahulUser.id } },
        session: { connect: { id: mentorshipSession.id } },
        approvedAt: new Date(),
        approver: { connect: { id: generalManager.id } },
        has_rsvp: true,
      },
  ...
  if (warehouseSession) {
    await prisma.volunteerSession.upsert({
      ...
      create: {
        volunteer: { connect: { id: aishaUser.id } },
        session: { connect: { id: warehouseSession.id } },
        approvedAt: addDays(today, -20),
        approver: { connect: { id: generalManager.id } },
        has_rsvp: true,
        has_attended: true,
  ...
  const projectFeedback =
    ...
    (await prisma.volunteerProjectFeedback.create({
      data: {
        project: { connect: { id: warehouseProject.id } },
        overallRating: 5,
        managementRating: 5,
        planningRating: 4,
        resourcesRating: 4,
  ...
  const existingPeerFeedback = await prisma.peerFeedback.findUnique({
    where: {
  ...
  if (!existingPeerFeedback) {
    await prisma.peerFeedback.create({
      data: {
        reviewer: { connect: { id: aliciaUser.id } },
        reviewee: { connect: { id: rahulUser.id } },
        project: { connect: { id: mentorshipProject.id } },
        score: 4,
        type: PartnerFeedbackType.peer,
  ...
  const financeManager = requireUser('finance@siloam.org');

  const clinicProject = await getOrCreateDonationProject({
    projectManager: { connect: { id: financeManager.id } },
  ...
  const donorDaniel = requireUser('daniel.donor@siloam.org');
  const donorMei = requireUser('mei.donor@siloam.org');
  ...
  const recurringDonation =
    ...
    (await prisma.recurringDonation.create({
      data: {
        donor: { connect: { id: donorDaniel.id } },
        project: { connect: { id: clinicProject.id } },
  ...
  if (!existingTransaction) {
    await prisma.donationTransaction.create({
      data: {
        donor: { connect: { id: donorDaniel.id } },
        project: { connect: { id: clinicProject.id } },
        recurringDonation: { connect: { id: recurringDonation.id } },
        type: DonationType.individual,
  ...
  if (!existingTransactionTwo) {
    await prisma.donationTransaction.create({
      data: {
        donor: { connect: { id: donorMei.id } },
        project: { connect: { id: scholarshipProject.id } },
  ...
  const campaign =
    ...
    (await prisma.emailCampaign.create({
      data: {
        ...
        creator: { connect: { id: generalManager.id } },
        audienceFilter: {
          create: {
            projectId: mentorshipProject.id,
